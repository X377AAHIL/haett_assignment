...
class ReportGenerator:
    ...
    def generate_reports(self) -> dict:
        ...
        if not os.path.exists(self.reference_path):
            raise FileNotFoundError(
                f"Reference data not found at {self.reference_path}"
            )

        if not os.path.exists(self.production_path):
            raise FileNotFoundError(
                f"Production data not found at {self.production_path}"
            )
        ...
        logger.info(
            f"Generating reports for {len(ref_data)} reference vs {len(prod_data)} production samples."
        )
        ...
        # MLflow Integration
        self._log_to_mlflow(
            html_path, json_path, self.reference_path, self.production_path
        )
        ...
        try:
            ...
            share_of_drifted_columns = drift_result.get("share_of_drifted_columns", 0.0)
            drifted_features = drift_result.get("number_of_drifted_columns", 0)
    ...
    def _log_to_mlflow(
        self, html_path: str, json_path: str, ref_path: str, prod_path: str
    ):
        ...
        try:
            ...
            if mlflow.active_run() is None:
                ...
                with mlflow.start_run(
                    run_name=f"drift_check_{datetime.datetime.now().strftime('%Y%m%d')}"
                ):
